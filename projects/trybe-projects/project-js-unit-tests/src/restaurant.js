...
const createMenu = (obj) => ({
  ...
  consuption: [],
  order(...ordersArr) {
    [...ordersArr].forEach((order) => this.consuption.push(order));
    return this;
  },
  pay() {
    const bill = [];
    const laCarteObj = { ...this.fetchMenu().food, ...this.fetchMenu().drink };
    const laCarteArr = [].concat(...Object.entries(laCarteObj));
    this.consuption.forEach((order) =>
      laCarteArr.find((menuItem) => {
        if (menuItem === order) return bill.push(laCarteObj[`${order}`]);
        return false;
      }));

    return bill.reduce((acc, cur) => acc + cur) * 1.1;
  },
});
